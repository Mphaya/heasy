...
if((@ARGV + 0) != 3) {
  ...
  print "	<mlf_words_ref>	- reference mlf created from transcriptions\n";
  print "	<mlf_words_rec>	- mlf with recognized words\n";
  print "	<hvite_log>	- log from hvite decode\n";
  exit 1;
}

my ($mlf_words_ref, $mlf_words_rec, $log_words_rec) = @ARGV;

# Read in all reference transcriptions
# -----------------------------------------------------------------------------
my %truth;

open MLF_REF, "$mlf_words_ref" or die "Can't open '$mlf_words_ref' for reading!";
while(<MLF_REF>) {
  chomp;
  $_ = NFC($_);
  if (/^"(.*)\.(lab|rec)"$/) {
...
open MLF_REC, "$mlf_words_rec" or die "Can't open '$mlf_words_rec' for reading!";
while(<MLF_REC>) {
  chomp;
  $_ = NFC($_);
  if (/^"(.*)\.(lab|rec)"$/) {
...
# -----------------------------------------------------------------------------
my %from_log;
open LOG, "$log_words_rec" or die "Can't open '$log_words_rec' for reading!\n";
while(<LOG>) {
  chomp;
  $_ = NFC($_);
  if (/^File:(.*).mfc\s*$/) {
